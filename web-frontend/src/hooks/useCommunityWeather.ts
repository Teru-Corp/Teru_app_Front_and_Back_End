...
export const useCommunityWeather = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const { condition, temperature, emotion, count, avgX, avgY } = res.data;
            ...
            setData({
                ...
                avgX,
                avgY,
                colors: CONDITION_COLORS[weatherCondition] || CONDITION_COLORS.Cloudy,
                teruFeeling: getTeruFeeling(weatherCondition, emotion || 'calm')
            });
            setError(null);
        } catch (e) {
